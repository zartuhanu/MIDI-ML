...
# ====== EDIT ======
TOKENS_TXT   = Path("token_dataset/tokens.txt")
LABELS_CSV   = Path("token_dataset/labels_humans_cleaned_v1.csv")
OUT_DIR      = Path("token_dataset")
VOCAB_JSON   = OUT_DIR / "vocab.json"
IDS_JSONL    = OUT_DIR / "ids.jsonl"
...
def _canonicalize_key_token(tok: str):
    ...
    body = body.replace(" ", "")
    body = body.replace("_", "")  # e.g., E-m -> Em (we'll re-insert the accidental)
    # Extract root letter
    if not body:
        return None
    root = body[0].upper()
    ...
    rest = body[1:]
    # minor flag if trailing 'm' (case-insensitive)
    ...
    accidental = ""
    if rest:
        # Accept one accidental symbol from map
        sym = rest[0]
        accidental = _ACCIDENTAL_MAP.get(sym, "")
        # ignore anything else beyond 1 accidental (defensive)
        # if there's more junk, drop token
        if accidental == "" and sym:
            # allow 'b' written as '-': we handled above; anything else -> invalid
            return None
...
def main():
    ...
    if n_tokens != n_labels:
        print(f"⚠️  tokens.txt lines ({n_tokens}) != labels rows ({n_labels})")
        if n_tokens > n_labels:
    ...
    def safe_preview_index(k):
        return k if 0 <= k < min_len else None
    ...
    # Diagnostics
    if dropped_misc:
        print(f"\nℹ️ dropped tokens during normalization: {dropped_misc}")
    if dur_before:
        def _topn(c, n=8): 
