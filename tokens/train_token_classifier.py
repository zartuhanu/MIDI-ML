...
# IMPORTANT:
#   - Keep LABELS_CSV pointing to the ORIGINAL file (row order for splits)
#   - CLEANED_LABELS_CSV is an overlay (json_path -> cleaned mood). If present, we use it.
LABELS_CSV         = DATA_DIR / "labels_human_merged.csv"
CLEANED_LABELS_CSV = DATA_DIR / "labels_humans_cleaned_v1.csv"

# Use the original frozen splits (indices refer to rows in LABELS_CSV)
...
# Balanced sampling toggle (helps minority classes)
USE_SAMPLER = True
# ==============================
...
def read_ids():
    ...
    with IDS_JSONL.open("r", encoding="utf-8") as f:
        for ln in f:
            ...
            jp = obj["json_path"]
            vec = obj["ids"]
            # index by both original path and basename (robust to mismatches)
            ids[jp] = vec
...
def read_labels_with_overlay():
    """
    Read original labels to preserve row order for splits.
    Load cleaned labels as an overlay (json_path -> mood).
    Build label2id from the effective mood set (cleaned if present).
    """
    base_rows = []
...
class TinySelfAttention(nn.Module):
    """1-head self-attention over time, very light."""
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            att = att.masked_fill(mask.unsqueeze(1)==0, -1e4)
...
def evaluate(model, loader, device, n_classes, amp_ctx):
    ...
    for c in range(n_classes):
        tp = int(((preds==c) & (trues==c)).sum())
        fp = int(((preds==c) & (trues!=c)).sum())
        fn = int(((preds!=c) & (trues==c)).sum())
...
def build_items(rows, ids_map, label2id, idx_list, split_name, cleaned_map=None):
    ...
    for i in idx_list:
        ...
        if cleaned_map is not None:
            if jp not in cleaned_map:
                dropped_not_in_clean += 1
                continue  # filtered out by cleaning
            mood = cleaned_map[jp]
        else:
            mood = rows[i]["mood"]

        if EXCLUDE_NEUTRAL and mood == "neutral":
            continue
        if mood not in label2id:
            continue

        key = jp if jp in ids_map else Path(jp).name
        if key not in ids_map:
...
def main():
    ...
    if USE_SAMPLER:
        ...
        sample_weights = [weights_per_class[l] for l in labels_vec]
    ...
    assert n_classes >= 2, "Need at least 2 classes after filtering."
    model = FastTokenClassifier(vocab_size, n_classes, pad_id).to(device)
    weights = compute_class_weights(items_train, n_classes).to(device)
    ...
    for ep in range(1, EPOCHS+1):
        ...
        # Eval on val
        val_metrics = evaluate(model, dl_va, device, n_classes, amp_ctx)
        ...
        if val_metrics["macro_f1"] > best_f1:
            ...
            torch.save({
                ...
                "vocab_size": len(stoi),
                "config": {"D_MODEL":D_MODEL,"MAX_LEN":MAX_LEN,"EXCLUDE_NEUTRAL":EXCLUDE_NEUTRAL,
                           "USE_TINY_ATTENTION":USE_TINY_ATTENTION}
            }, ckpt_path)
    ...
    test_metrics = evaluate(model, dl_te, device, n_classes, amp_ctx)
    print("\n=== TEST RESULTS ===")
    ...
    for lbl, idx in id2label_sorted:
        ...
        print(f"  {lbl:12s}  F1={m['f1']:.3f}  (P={m['precision']:.3f}, R={m['recall']:.3f}, n={m['support']})")

    # Export embeddings (pooled)
    all_items = items_train + items_val + items_test
    collate_all = PadCollate(pad_id=pad_id, max_len=MAX_LEN)
    dl_all = DataLoader(TokenDataset(all_items, pad_id, MAX_LEN), batch_size=BATCH_SIZE, shuffle=False,
                        collate_fn=collate_all, num_workers=0, pin_memory=(devtag=="cuda"))
    song_ids = [(it["json_path"], it["split"]) for it in all_items]
    export_embeddings(model, dl_all, device, OUT_DIR/"embeddings.npy", song_ids, pad_id, amp_ctx)
