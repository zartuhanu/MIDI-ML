...
# ============== SETTINGS ==============
LABELS_CSV       = Path("token_dataset/labels_human_merged.csv")
SPLITS_JSON      = Path("token_dataset/splits.json")
TEST_ARTISTS_TXT = Path("token_dataset/test_artists.txt")

TEST_RATIO       = 0.20       # ~20% of artists → TEST
VAL_FRACTION_OF_REST = 0.125  # ≈10% overall (0.8 * 0.125)

HASH_SEED        = "v1_freeze"    # change ONLY if you intentionally want a brand-new frozen test
MAX_TEST_SONGS_PER_ARTIST = 15    # <-- per-artist cap in TEST
...
def warn_artist_dominance(by_artist, test_artists, test_indices_total):
    ...
    for a in sorted(test_artists):
        n = len(by_artist[a])
        # Cap mirrors the selection applied to indices to reflect true contribution
        n_capped = min(n, MAX_TEST_SONGS_PER_ARTIST)
...
def main():
    ...
    # Freeze test artists (stable hash; unbiased by manual labeling coverage)
    ...
    def indices_for(arts, cap=False):
        ...
        for a in arts:
            ...
            if cap and len(songs) > MAX_TEST_SONGS_PER_ARTIST:
                # deterministic subset under cap
